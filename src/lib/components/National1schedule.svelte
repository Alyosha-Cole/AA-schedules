...
<style>
  ...
  @media print {
    ...
    .col-drag-handle, .col-edit-btn { display: none !important; }
  }
</style>
    ...
    <thead>
      <tr>
        ...
        {#each columns as col (col.id)}
          <th 
            class="border-2 border-slate-900 bg-slate-700 text-white px-1 py-1 text-center font-bold drag-col {col.type === 'time' ? 'w-[85px] text-left' : ''} {colDragItem === col.id ? 'col-dragging' : ''} {colDragOverId === col.id ? 'col-drag-over' : ''}"
            draggable="true"
          ...
          >
            <div class="col-header-content">
              ...
              <button class="col-edit-btn print:hidden p-0.5 rounded hover:bg-slate-600 transition-colors" on:click|stopPropagation={() => openColumnModal(col)} title="Edit column">
